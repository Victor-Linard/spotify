<?php
    ...
    $search_bar_content = str_replace(" ","%20", $search_bar_content);
...
if((!isset($array_artists_id) && !isset($array_tracks_id) && !isset($array_albums_id)) || $run_api) {
    ...
    foreach ($json['artists']['items'] as $i => $artist) {
        ...
        if($save == 'True' && !verify_exit_record("artists", $id_artist)){
            $sql = 'INSERT INTO artists(name,id_artist,followers,image,popularity) VALUES(:name,:id_artist,:followers,:image,:popularity)';
    ...
    foreach ($json['tracks']['items'] as $i => $track) {
        ...
        if($save == 'True' && !verify_exit_record("tracks", $id_track)){
            $sql = 'INSERT INTO tracks(id_track,name,duration,id_artist,image) VALUES(:id_track,:name,:duration,:id_artist,:image)';
    ...
    foreach ($json['albums']['items'] as $i => $album) {
        ...
        if($save == 'True' && !verify_exit_record("albums", $album_id)){
            $sql = 'INSERT INTO albums(id_album,name,id_artist,image) VALUES(:album_id,:name,:artist_id,:image)';
...
$message = $run_api ? "Data from Spotify" : "Data from local databases";
